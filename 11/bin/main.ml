...
class monkey id items operation test (passed : int) (failed : int) =
  object (self)
    ...
    method push item = stack <- item :: stack
    ...
    method empty = List.length stack == 0
...
class monkey_builder =
  object
    ...
    method set_id identifier = id <- identifier
    method print_id = Printf.printf "id: %d\n" id
    method set_items new_items = items <- new_items
    method set_operation new_operation = operation <- new_operation
    method set_test new_test = test <- new_test
    method set_passed new_passed = passed <- new_passed
    method set_failed new_failed = failed <- new_failed
    method build = new monkey id items operation test passed failed
  end ;;
  ...
  object (self)
    ...
    method get_test_mul =
      List.fold_left
        ...
        1
        (List.map (fun monkey -> monkey#get_test) !monkeys)
...
let create_monkeys lines =
  ...
  let input_handler i line =
    ...
    | 0 ->
        (* -1 to remove ":" *)
        let id =
          int_of_string (String.sub line ptr_position (String.length line - ptr_position - 1))
        in
    ...
    | 2 ->
        ...
        let op x y = match List.nth operation_list 1 with "*" -> x * y | "+" -> x + y | _ -> -1 in
        let parse old value = match value with "old" -> old | _ -> int_of_string value in
        let operation old =
    ...
    | 3 ->
        let test =
          int_of_string (String.sub line ptr_position (String.length line - ptr_position))
        in
